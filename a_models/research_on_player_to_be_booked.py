...
def roptbb_today(day0, day1, space):  
    # ----------------------------------------------------------   
    space += "__"
    # ---------------------------------------------------------- 
    print(space + "roptbb_today()", flush=True)
    # ----------------------------------------------------------  
    host="localhost"
    user="root" 
    ...
    query += " and research_on_player_to_be_booked_updated_at is null " 
    ...
    for x in result:    
        ...
        # ------------------------------------------------------
        roptbb_odds(leagueapi_id, season, fixtureapi_id, space)
    # ---------------------------------------------------------- 

def roptbb_get(leagueapi_id, today, space):  
    # ----------------------------------------------------------   
    space += "__"
    # ---------------------------------------------------------- 
    print(space + "roptbb_get()", flush=True)
    # ----------------------------------------------------------  
    host="localhost"
    user="root" 
    database="pr_mmbuzz_2022_06"
...
def roptbb_odds(leagueapi_id, season, fixtureapi_id, space):  
    # ----------------------------------------------------------   
    space += "__"
    # ---------------------------------------------------------- 
    print(space + "roptbb_odds()", flush=True)
    # ----------------------------------------------------------  
    host="localhost"
    user="root" 
    database="pr_mmbuzz_2022_06"
    ...
    query += " , season  " 
    query += " , fixtureapi_id  "
    query += " , pre_player_to_be_booked  "
    query += " , end_player_to_be_booked  "
    ...
    for x in result:    
        ...
        fixtureapi_id   = str(x[2])  
        # ------------------------------------------------------
        pre_player_to_be_booked     = str(x[3])  
        end_player_to_be_booked     = str(x[4])  
        # ------------------------------------------------------
        # word = space + "[" + str(counter) + "/" +str(total_rows) + "] " 
        # word += " #" + leagueapi_id 
        # word += " - " + season 
        ...
        # print(word, flush=True)    
        # ------------------------------------------------------ 
        array_pre_player_to_be_booked = pre_player_to_be_booked.split(";")
        roptbb_array(array_pre_player_to_be_booked, 'pre', leagueapi_id, season, fixtureapi_id, space)        
        # ------------------------------------------------------  
        array_end_player_to_be_booked = end_player_to_be_booked.split(";")
        roptbb_array(array_end_player_to_be_booked, 'end', leagueapi_id, season, fixtureapi_id, space)
        # ------------------------------------------------------  
        ...
        if(x[4] is not None):
            ...
            # --------------------------------------------------    
            query_commit += " `research_on_player_to_be_booked_updated_at` = now() "
            # --------------------------------------------------    
...
def roptbb_array(array_AGS, prep, leagueapi_id, season, fixtureapi_id, space):  
    # ----------------------------------------------------------   
    space += "__"
    # ---------------------------------------------------------- 
    print(space + "roptbb_array()", flush=True)
    # ----------------------------------------------------------  
    for row in array_AGS:
        ...
        if(len(value_and_odd) == 2): 
            ...
            if(float(odd) <= 4.75):
                ...
                # ---------------------------------------------- 
                roptbb_insert_or_update(leagueapi_id, season,
                            fixtureapi_id,
                            value,
                            odd,
                            prep,
...
def roptbb_insert_or_update(leagueapi_id, season,
                            fixtureapi_id,
                            value,
                            odd,
                            prep,
    ...
    space += "__"
    # ---------------------------------------------------------- 
    print(space + "roptbb_insert_or_update()", flush=True)
    # ----------------------------------------------------------  
    host="localhost"
    user="root" 
    database="pr_mmbuzz_2022_06"
    ...
    # ----------------------------------------------------------  
    query = " Select  * " 
    query += " from research_on_player_to_be_booked "    
    query += " where leagueapi_id = "+str(leagueapi_id)+" "    
    query += " and season = "+str(season)+" "    
    query += " and fixtureapi_id = "+str(fixtureapi_id)+" "    
    ...
    if(total_rows == 0): 
        # ------------------------------------------------------ 
        query_commit = "INSERT INTO `research_on_player_to_be_booked`( "
        # ------------------------------------------  
    ...
    # ----------------------------------------------------------  
        ...
        # ------------------------------------------------------  
        query_commit = "UPDATE `research_on_player_to_be_booked` SET "
        # ------------------------------------------------------    
        if(prep == "pre"): 

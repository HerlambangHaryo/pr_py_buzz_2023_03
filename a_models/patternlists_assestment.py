...
def pa_assestment_daily_update(leagueapi_id, league_country, 
    ...
    space += "__"
    # ---------------------------------------------------------- 
    print(space + "pa_assestment_daily_update()", flush=True)
    # ----------------------------------------------------------  
    host="localhost"
    user="root" 
    ...
    if(pre_ah_pattern is None):
        query += " and pre_ah_pattern = Null "  
    else:
        query += " and pre_ah_pattern = '"+str(pre_ah_pattern)+"' "  

    if(pre_ah_pattern_mirror is None):
        query += " and pre_ah_pattern_mirror = Null "  
    else:
        query += " and pre_ah_pattern_mirror = '"+str(pre_ah_pattern_mirror)+"' "  

    if(pre_gou_pattern is None):
        query += " and pre_gou_pattern = Null "  
    else:
        query += " and pre_gou_pattern = '"+str(pre_gou_pattern)+"' "  
 

    if(end_ah_pattern is None):
        query += " and end_ah_pattern = Null "  
    else:
        query += " and end_ah_pattern = '"+str(end_ah_pattern)+"' "  

    if(end_ah_pattern_mirror is None):
        query += " and end_ah_pattern_mirror = Null "  
    else:
        query += " and end_ah_pattern_mirror = '"+str(end_ah_pattern_mirror)+"' "  

    if(end_gou_pattern is None):
        query += " and end_gou_pattern = Null "  
    else:
        query += " and end_gou_pattern = '"+str(end_gou_pattern)+"' "  
    
    query += " and `goals_home` is not null " 
    query += " and `goals_away` is not null "  
    query += " and `fixture_status` in ('Match Finished Ended', 'Match Finished') "
    # ----------------------------------------------------------   
    space += "__"
    # ----------------------------------------------------------  
    ...
    # ----------------------------------------------------------  
    if(total_rows != 0):
        pa_final_assestment(result, leagueapi_id, league_country, 
                pre_ah_pattern, pre_ah_pattern_mirror, pre_gou_pattern, 
                end_ah_pattern, end_ah_pattern_mirror, end_gou_pattern, space)
    # ----------------------------------------------------------  
    # ----------------------------------------------------------

def pa_final_assestment(myresult, leagueapi_id, league_country, 
            pre_ah_pattern, pre_ah_pattern_mirror, pre_gou_pattern, 
            end_ah_pattern, end_ah_pattern_mirror, end_gou_pattern, space): 
    # ----------------------------------------------------------   
    space += "__"
    # ---------------------------------------------------------- 
    print(space + "pa_final_assestment()", flush=True)
    # ----------------------------------------------------------  
    host="localhost"
    user="root" 
    database="pr_mmbuzz_2022_06"
    ...
    space += "__" 
    # -------------------------------------------------- 
    print(space + "Total Row(s) : " + str(pattern_total_ff), flush=True)    
    # -------------------------------------------------- 
    for x in myresult: 
        # ---------------------------------------------- 
        btts_sts = 'no'
        ...
        goals_away = x[1]
        # ----------------------------------------------
        score_halftime_home = x[2]
        score_halftime_away = x[3]
        # ---------------------------------------------- 
        # print(space + "goals_home: " + str(goals_home) , flush=True)
        # print(space + "goals_away: " + str(goals_away) , flush=True)
        # ---------------------------------------------- 
    ...
    # --------------------------------------------------
    query += " `tanggal_update_patternlist_at` = current_timestamp " 
    # --------------------------------------------------
    query += " where `leagueapi_id` = '"+str(leagueapi_id)+"' "
    # --------------------------------------------------
    ...
    mydb.commit()     
    # --------------------------------------------------
    print(space + "patternlists updated", flush=True)
    # ---------------------------------------------------------- 

def pa_assestment(leagueapi_id, yesterday_ago, space): 
    # ----------------------------------------------------------  
    space += "  "
    print(space + "__pa_assestment__", flush=True)
    # ----------------------------------------------------------    
    space += "  "
    print(space + "_>pa_check_league_tanggal_update_patternlists__", flush=True)
    # ----------------------------------------------------------  
    space += "  "
    # ----------------------------------------------------------  
    host="localhost"
    ...
    # ---------------------------------------------------------- 
    space += "  "
    print(space + "_>pa_get_league_fixtures__", flush=True)
    # ---------------------------------------------------------- 
    query = ' SELECT  '
    ...
    myresult = mycursor.fetchall()
    # ----------------------------------------------------------    
    print(space + "Total Row(s) : " + str(len(myresult)), flush=True)  
    # ---------------------------------------------------------- 
    counter = 0 
    space += "  "
    # ---------------------------------------------------------- 
    for x in myresult:      
        ...
        # ------------------------------------------------------    
        print(space + "Total Row(s) : " + str(len(myresult_ff)), flush=True)  
        # ------------------------------------------------------ 
        pattern_total_ff = len(myresult_ff)
        # ------------------------------------------------------  
        print(space + "_>pa_check_patternlist__", flush=True)
        # ------------------------------------------------------ 
        host="localhost"
        user="root" 
        database="pr_mmbuzz_2022_06"
        ...
        if(pattern_total_pl == 0 ): 
            # --------------------------------------------------  
            print(space + "  _>pa_insert_patternlists__", flush=True)
            # -------------------------------------------------- 
            ...
            mycursor.execute(query)
            mydb.commit() 
            print(space + "insert commited", flush=True)
        # ------------------------------------------------------  
        host="localhost"
        user="root" 
        database="pr_mmbuzz_2022_06"
        ...
        if(pattern_total_ff < 1):  
            # --------------------------------------------------
            print(space + "  Pattern Total tidak memenuhi syarat ", flush=True)
        else: 
            # --------------------------------------------------
            print(space + "  Memenuhi syarat ", flush=True) 
            ## -----------------------------------////////////// betid = 1
            match_winner_home = 0
            match_winner_draw = 0
            match_winner_away = 0
            ...
            for x in myresult_ff: 
                ...
                # print(space + "goals_away: " + str(goals_away) , flush=True)
                # ---------------------------------------------- 
                # print(space + "score_halftime_home: " + str(score_halftime_home) , flush=True)
                # print(space + "score_halftime_away: " + str(score_halftime_away) , flush=True)
                # ----------------------------------------------
                total_goals = goals_home + goals_away
                # ---------------------------------------------- 
                if(score_halftime_home is not None):
